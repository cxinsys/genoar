...
def was_released(sample_dir: Path) -> bool:
    """Whether this sample's BAM was deliberately removed after being verified.

    Retention releases the BAM of a completed sample, which leaves exactly the
    shape this module reads as failure: a cellranger_output with no BAM in it.
    Acting on that deleted the matrices and summaries the rotation was
    performed FOR, and moved the sample out of success/ -- on the second run
    over a rotated tree, which is the documented way to pick up stragglers.
    """
    try:
        with open(sample_dir / RECEIPT_NAME) as fh:
            receipt = json.load(fh)
    except (OSError, ValueError):
        return False
    note = receipt.get("released")
    return isinstance(note, dict) and bool(note.get("removed"))


def is_failed(sample_dir: Path) -> bool:
    cr = sample_dir / "cellranger_output"
    bam = cr / "outs" / "possorted_genome_bam.bam"
    # If cellranger_output exists but no outs/bam, consider failed -- unless a
    # run took the BAM away on purpose and said so.
    if cr.exists() and not bam.exists():
        return not was_released(sample_dir)
    return False
